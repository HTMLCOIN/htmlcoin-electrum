...
;Variables
  ...
  !define PRODUCT_NAME "Htmlcoin-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/HTMLCOIN/htmlcoin-electrum"
  !define PRODUCT_PUBLISHER "Htmlcoin Team"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\Htmlcoin-electrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\Htmlcoin-electrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\Htmlcoin-electrum-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\Htmlcoin-electrum-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\Htmlcoin-electrum-${PRODUCT_VERSION}.exe" 0


  ;Links htmlcoin: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\htmlcoin" "" "URL:htmlcoin Protocol"
  WriteRegStr HKCU "Software\Classes\htmlcoin" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\htmlcoin" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\htmlcoin\shell\open\command" "" "$\"$INSTDIR\Htmlcoin-electrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\htmlcoin"
