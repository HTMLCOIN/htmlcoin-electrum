...
class Huobi(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.huobi.pro', '/market/trade?symbol=htmlcoin{}'.format(ccy.lower()))
...
class OKEX(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('www.okex.com', '/api/v1/ticker.do?symbol=htmlcoin_{}'.format(ccy.lower()))
...
class Binance(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.binance.com', '/api/v3/avgPrice?symbol=HTMLCOIN{}'.format(ccy.upper()))
...
class BitStamp(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        if ccy in CURRENCIES[self.name()]:
            json = await self.get_json('www.bitstamp.net', f'/api/v2/ticker/htmlcoin{ccy.lower()}/')
...
class CoinCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coincap.io', '/v2/rates/htmlcoin/')
    ...
    async def request_history(self, ccy):
        ...
        history = await self.get_json('api.coincap.io',
                                      '/v2/assets/htmlcoin/history?interval=d1&limit=2000')
...
class CoinGecko(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        json = await self.get_json('api.coingecko.com', '/api/v3/coins/htmlcoin/market_chart?vs_currency=%s&days=0' % ccy)
    ...
    async def request_history(self, ccy):
        history = await self.get_json('api.coingecko.com',
                                      '/api/v3/coins/htmlcoin/market_chart?vs_currency=%s&days=max' % ccy)
...
class CoinMarketCap(ExchangeBase):
    ...
    async def get_rates(self, ccy):
        ...
        json = await self.get_json('api.coinmarketcap.com', '/v2/ticker/1684/?convert={}'.format(ccy))
